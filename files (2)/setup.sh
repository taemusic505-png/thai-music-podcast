...
# 🎓 Classroom Management System - Quick Setup Script
# ใช้งาน: bash setup.sh
...
echo "✅ Node.js พบ: $(node --version)"
echo "✅ NPM พบ: $(npm --version)"
echo ""

# สร้าง React App
echo "📦 กำลังสร้าง React App..."
npx create-react-app classroom-manager

cd classroom-manager

echo "📝 กำลังสร้าง package.json dependencies..."
npm install recharts lucide-react

# สร้าง .env.local
echo ""
echo "🔑 กำลังสร้าง .env.local ไฟล์..."
cat > .env.local << EOF
# Google Sheets API Key
# ได้จาก: https://console.cloud.google.com/apis/credentials
REACT_APP_GOOGLE_API_KEY=YOUR_API_KEY_HERE

# Google Sheet ID (แยกจาก URL)
# เช่น: https://docs.google.com/spreadsheets/d/1ABC123XYZ/edit
REACT_APP_SHEET_ID=YOUR_SHEET_ID_HERE
EOF

echo "✅ สร้าง .env.local แล้ว"
echo "⚠️  โปรดแก้ไข .env.local ด้วย API Key ของคุณ"
echo ""

# สร้าง src/App.jsx
echo "📝 กำลังอัปเดต App.jsx..."
cat > src/App.jsx << 'EOF'
...
export default function ClassroomManager() {
  ...
  const fetchSheetData = async (sheetId, sheetName) => {
    try {
      ...
      if (!apiKey) {
        throw new Error('กรุณาตั้งค่า REACT_APP_GOOGLE_API_KEY ใน .env.local');
      }
  ...
  const handleAddAttendance = async (e) => {
    e.preventDefault();
    alert('✅ ฟีเจอร์เพิ่มข้อมูล: ปัจจุบันสามารถแก้ไข Google Sheet โดยตรงได้');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        {loading && <div className="text-center py-8"><p>⏳ กำลังโหลดข้อมูล...</p></div>}
...
echo "✅ อัปเดต App.jsx แล้ว"
echo ""

# สร้าง .gitignore (ป้องกัน API Key ถูก push ขึ้น GitHub)
...
echo "✅ สร้าง .gitignore แล้ว"
echo ""

# สร้าง README
echo "📝 กำลังสร้าง README.md..."
cat > README.md << 'EOF'
# 📚 Classroom Management System

ระบบจัดการชั้นเรียนฟรีที่ใช้ Google Sheets เป็นฐานข้อมูล

## ✨ ฟีเจอร์
- ✅ ตรวจสอบการเข้าเรียนเป็นรายวิชา
- ✅ บันทึกและติดตามงานนักเรียน
- ✅ แสดงสถิติด้วยกราฟและตัวเลข
- ✅ ใช้งานฟรี (ไม่มีค่าใช้งาน)
...
2. **ตั้งค่า .env.local:**
   - ใช้ API Key จาก Google Cloud Console
   - ใส่ Google Sheet ID
...
## 📚 เอกสารเพิ่มเติม
ดูไฟล์ `SETUP_GUIDE_TH.md` สำหรับคำแนะนำโดยละเอียด

## 📄 License
MIT - ใช้งานอย่างอิสระในสถานศึกษา
EOF

echo "✅ สร้าง README.md แล้ว"
echo ""
...
echo ""
echo "📝 ขั้นตอนต่อไป:"
echo "1. แก้ไขไฟล์ .env.local ด้วย API Key ของคุณ"
echo "2. รันคำสั่ง: npm start"
echo "3. เปิดเบราว์เซอร์: http://localhost:3000"
echo ""
echo "📚 คู่มือการตั้งค่า: ดูไฟล์ SETUP_GUIDE_TH.md"
echo ""
EOF